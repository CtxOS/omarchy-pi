...
check "test -f $REPO/crate/pi-image/lib/firstboot.sh" "firstboot.sh exists"
check "test -x $REPO/crate/pi-image/lib/firstboot.sh" "firstboot.sh executable"
check "grep -q 'set -eEo pipefail' $REPO/install.sh" "install.sh has set -eEo pipefail enabled"
check "! test -f $REPO/install/iso.sh" "install/iso.sh dead stub removed"
check "grep -q 'OMARCHY_PATH' $REPO/install/login/plymouth.sh" "plymouth.sh uses OMARCHY_PATH"
check "grep -q '\-\-country' $REPO/install/helpers/set-arm-mirrors.sh" "set-arm-mirrors.sh has --country flag"
check "grep -q 'firstboot' $REPO/crate/pi-image/build.sh" "build.sh calls firstboot.sh"
check "grep -q 'firstboot' $REPO/crate/pi-image/lib/firstboot.sh" "firstboot.sh has firstboot logic"
check "grep -q '\-\-pi' $REPO/bin/omarchy-theme-set" "omarchy-theme-set has --pi option"
check "grep -q 'OMARCHY_PATH' $REPO/bin/omarchy-theme-set" "omarchy-theme-set uses OMARCHY_PATH"
check "! grep -v '^#' $REPO/install/omarchy-base.packages | grep -q 'linux-asahi\|asahi-fwextract\|asahi-desktop-meta\|grub\|efibootmgr'" "omarchy-base.packages has no Asahi/UEFI packages"
check "grep -q 'yay-bin' $REPO/install/omarchy-base.packages" "omarchy-base.packages uses yay-bin"
check "! grep -v '^#' $REPO/install/omarchy-other.packages | grep -q 'broadcom-wl\|limine\|nvidia-dkms\|macbook12\|linux-t2\|apple-\|libva-intel\|libva-nvidia'" "omarchy-other.packages has no x86_64-only packages"
check "! grep -q 'arch-mact2' $REPO/install/post-install/pacman.sh" "post-install/pacman.sh has no arch-mact2 repo"
